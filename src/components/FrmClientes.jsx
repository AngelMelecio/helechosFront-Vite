...
const FrmClientes = ({
  ...
  const formik = useFormik({
    ...
    validate,
    onSubmit: values => {
      values.contactos=objCliente.contactos
  ...
  const handleDeleteContacto = (e, indx) => {
    e.preventDefault()
    if (objCliente.contactos.length === 1) {
      setObjCliente(prev => ({ ...prev, contactos: [{ ...contactoObj }] }))
      return
    }
    let newContacts = [...objCliente.contactos]
    newContacts.splice(indx, 1)
    setObjCliente(prev => ({ ...prev, contactos: newContacts }))
  }
  ...
  const handleFocusContacto = (e, indx) => {
    if (indx === objCliente.contactos.length - 1) {
      setObjCliente(prev => ({ ...prev, contactos: [...prev.contactos, { ...contactoObj }] }))
    }
  }
                  ...
                  <div className='flex flex-col w-full' >
                    <table className="w-full">
                      ...
                      <tbody>
                        {
                          objCliente?
                          objCliente.contactos.map((c, i) => <tr key={'C' + i} className="array-row">
                            <td>
                              <input
